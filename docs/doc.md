...
Все race conditions устранены, утечки ресурсов закрыты, система готова к продакшену.
...
- Обрабатывает результаты детектора:
  - **CAPTCHA** → решает через `resolve_captcha_flow()`, если не решена → возврат задачи
...
**В каталоге:**
- Блокирует прокси через `block_proxy()`
- Закрывает браузер через `browser.cleanup_browser()`
- Обнуляет переменные: `current_proxy_id`, `playwright_instance`, `browser_instance`, `context_instance`, `page_instance`
- Возвращает задачу в очередь через `return_task_to_queue(increment_retry=True)`
- Продолжает цикл → берет новый прокси и задачу

**В детальном парсинге:**
- Аналогично: блокирует прокси, cleanup браузера, возврат задачи
...
- Логирует ошибку
- Ждет `NO_PROXIES_WAIT` секунд (по умолчанию 60)
- Повторяет попытку
